...
def make_ts_file_angle(mybasis,myfile,myw,angle1,angle2,point,coor,run='minimize',charge=0,spinmult=1):
    basis='basis    '+mybasis
    rc_w='rc_w '+str(myw)
    myco=myfile+'.xyz'
    ...
    ts_file.write('\n')
    angle='angle '+str(angle1)+' '+str(angle2)+' '+str(point)+' '+str(coor)
    ts_file.write(angle)
    ts_file.write('\n')
    ts_file.write('$end')
    ts_file.write('\n')
...
def make_sh_file(myfile):
    ...
    for i, line in enumerate(sh):
        ...
        if i==19:
            openfile=myfile+'.ts'
            line2='terachem /scratch/woon/horizon/'+openfile
            line2=str(line2)
            f.write(line2)
            f.write('\n')
        if (i !=5 and i !=19):
...
myfile='optLS16'
myw='0.06'
angle1='83.99' #initial
angle2='123.99' #Final
point='9'
coor='16_15_10_9'

# Run angle scan
make_ts_file_angle(mybasis,myfile,myw,angle1,angle2,point,coor,run='minimize',charge=0,spinmult=1)
make_sh_file(myfile)
done=check_run_status()
time.sleep(10)
extractxyzfromscan(myfile)
time.sleep(10)
for i,line in enumerate(open('./mylistest.txt', 'r')):
    myfileES=str(line).strip('\n')
    make_ts_file_EST(mybasis,myfileES,myw,run='energy',charge=0,spinmult=1, cal='singlet')
    make_sh_file(myfileES)
    done,job_id=check_run_status()
    time.sleep(10)
    if float(angle1)<float(angle2):
        angle=float(angle1)+(abs(float(angle1)-float(angle2))/(int(point)-1))*i
    else:
        angle=float(angle1)-(abs(float(angle1)-float(angle2))/(int(point)-1))*i
    angle=str(angle)
    idname=str(job_id)
    extractexcitation(idname,myfile,angle,'singlet')
    make_ts_file_EST(mybasis,myfile,myw,run='energy',charge=0,spinmult=1, cal='triplet')
    make_sh_file(myfile)
    done,job_id=check_run_status()
    time.sleep(10)
    idname=str(job_id)
    extractexcitation(idname,myfile,angle,'triplet')



#need to auto positive and negative angle
#need to save scan-optimize
#need to delete file such for singlet and tripley
